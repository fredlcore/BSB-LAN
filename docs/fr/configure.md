...
|Économie d'énergie|`esp32_save_energy`|Réduit la vitesse, économise l'énergie. Ne pas activer lors de l'utilisation du Wi-Fi.|**Activé** (`true`)<br>**Désactivé** (`false`)|                        |
| Système de fichiers du serveur Web | `webserver` | Active le service de fichiers depuis la carte SD | **Activé** (`true`) <br>**Désactivé** (`false`) |
...
|**Nom du réseau WLAN**|`wifi_ssid`        |Définir le nom du réseau WLAN|**Nom_du_réseau** (`Nom_du_réseau`)|
|**Mot de passe WLAN**|`wifi_pass`         |Définir le mot de passe du WLAN|**MotDePasseDuWLAN** (`MotDePasseDuWLAN`)|
|Utilisation du DHCP|`use_dhcp`        |Définir si oui ou non utiliser DHCP|**Activé** (`true`)<br>**Désactivé** (`false`)|
...
|Actualiser la détection automatique MQTT|`MQTTRefAD`|Actualiser les entrées de détection automatique MQTT au cas où le broker aurait perdu ces informations.|**Activé** (`true`)<br>**Désactivé** (`false`)
...
| Utilisation de MAX ! | `enable_max_cul` | Activer ou désactiver la connexion à CUNO/CUNX/MAX!Cube modifié | **Activé** (`true`)<br>**Désactivé** (`false`)||
...
| Utilisation de IPWE | `enable_ipwe` | Activer l'extension IPWE (`/ipwe.cgi`) | **Activé** (`true`)<br>**Désactivé** (`false`) |
...
| Interface Web | Fichier de configuration | Fonctionnalité | Paramètres possibles |
| ------------- | --- | --- | --- |
| Utilisation du débogage | `debug_mode` | Sélectionne la sortie de débogage | **Désactivé** (`0`)<br>**Série** (`1`)<br>**Telnet** (`2`) |
| Mode verbeux | `verbose` | Active ou désactive les messages de débogage verbeux (ne pas désactiver sauf si conseillé) | **Activé**  (`1`)<br>**Désactivé** (`0`) |
| Mode données brutes | `monitor` | Active ou désactive le mode données brutes (ne pas activer sauf si conseillé) | **Activé**  (`1`)<br>**Désactivé** (`0`) |
| Afficher les paramètres inconnus | `show_unknown` | Essaye d'afficher aussi les paramètres non pris en charge par l'appareil de destination | **Activé** (`true`)<br>**Désactivé** (`false`) |
