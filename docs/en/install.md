...
If your controller is missing a proper plug for the connector, you might improvise with a [6.35mm flat connector sleeve](https://www.reichelt.com/de/en/shop/product/flat_plug_sleeve_fully_insulated_width_6_35_mm_blue-231369).  
...
1. If the connection between the controller and the BSB-LAN adapter is correct, then the red LED of the adapter should turn on and flicker occasionally. If it is not flickering occasionally, then check if the adapter has been accidentially connected to the `G+` pin of the controller. That pin provides power only, no data. If the LED remains off when connected to the LPB connectors, then the LPB power supply might need to be activated in the controller's LPB settings. 
