...
- **ATTENTION: (potentially) BREAKING CHANGE!** Connections to MQTT brokers on ports other than 1883 will use TLS and require a proper certificate on the remote server. If your remote broker uses a self-signed certificate, connections will fail. Connections to brokers on port 1883 will continue to be made without TLS.
