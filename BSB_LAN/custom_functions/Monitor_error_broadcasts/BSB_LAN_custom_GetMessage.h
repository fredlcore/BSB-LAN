...
if (((msg[2]==ADDR_ALL && bus->getBusType()==BUS_BSB) || (msg[2] >= 0xF0 && bus->getBusType()==BUS_LPB)) && msg[4+(bus->offset)]==TYPE_INF) { // Only handle broadcast messages
  ...
  if (cmd==0x0500006B) {
    ...
    if (error > 0) {
      ...
      printFmtToDebug("Heating system reports error no. %d with priority %d occurred!\r\n", msg[9+(bus->offset)], msg[10+(bus->offset)]); // First byte of payload of 0x0500006B is the error code
