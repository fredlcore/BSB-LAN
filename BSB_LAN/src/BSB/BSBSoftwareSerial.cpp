...
bool BSBSoftwareSerial::stopListening()
{
  if (active_object == this)
  {
    setRxIntMsk(false);
...
uint16_t BSBSoftwareSerial::subtract_cap(uint16_t num, uint16_t sub) {
  if (num > sub)
    return num - sub;
  else
    return 1;
...
void BSBSoftwareSerial::setRxIntMsk(bool enable)
{
    if (enable)
...
size_t BSBSoftwareSerial::write(uint8_t b)
{
  if (_tx_delay == 0) {
    setWriteError();
    return 0;
  ...
  uint8_t p = 0;	// parity
  uint8_t t;		// parity helper

  if (inv)
    b = ~b;

  for (t = 0x80; t; t >>= 1)	// calculate parity
    if (b & t) p++;
