...
void ICACHE_RAM_ATTR SPISlaveClass::_data_rx(uint8_t * data, uint8_t len)
{
    if(_data_cb) {
        _data_cb(data, len);
    }
...
void ICACHE_RAM_ATTR SPISlaveClass::_data_tx(void)
{
    if(_data_sent_cb) {
        _data_sent_cb();
    }
}
void ICACHE_RAM_ATTR SPISlaveClass::_status_tx(void)
{
    if(_status_sent_cb) {
        _status_sent_cb();
    }
}
void ICACHE_RAM_ATTR SPISlaveClass::_s_data_rx(void *arg, uint8_t * data, uint8_t len)
{
...
void ICACHE_RAM_ATTR SPISlaveClass::_s_data_tx(void *arg)
{
    reinterpret_cast<SPISlaveClass*>(arg)->_data_tx();
}
void ICACHE_RAM_ATTR SPISlaveClass::_s_status_tx(void *arg)
{
...
void SPISlaveClass::onData(void (*cb)(uint8_t *data, size_t len))
{
    _data_cb = cb;
}
void SPISlaveClass::onDataSent(void (*cb)(void))
{
    _data_sent_cb = cb;
}
...
void SPISlaveClass::onStatusSent(void (*cb)(void))
{
    _status_sent_cb = cb;
}
